...
# Law-Psychology Research in the U.S. {background-image="images/uslawpsych.png" background-opacity="0.2"}
...
::: incremental
> [Selective reviews with narrow focus]{.rn}

-   Experimental forensic psychology [Moore & Finn (1986)](https://pubmed.ncbi.nlm.nih.gov/3745469/)

-   Jury decision making [Devine et al (2000)](https://www.researchgate.net/publication/228688698_Jury_Decision_Making_45_Years_of_Empirical_Research_On_Deliberating_Groups)

-   Cross race effect [Meissner & Brigham (2001)](https://psycnet.apa.org/record/2001-14540-001)
:::
...
::: incremental
> [Approved by AP-LS committee]{.rn}

-   Eyewitness Identification Procedures: Recommendations for lineups and Photospreads [Wells et al (1998)](https://psycnet.apa.org/fulltext/2020-06220-002.html)

-   Police-induced Confessions: Risk Factors and Recommendations [Kassin et al (2010)](https://psycnet.apa.org/record/2010-02823-002)

-   Policy and Procedure Recommendations for the Collection and Preservation of Eyewitness Identification Evidence [Constanzo & Levett (2020)](https://psycnet.apa.org/fulltext/2020-06220-001.html)
:::
...
# Create the plot
ggpm <- ggplot(usa) +
  geom_sf(aes(fill = ID == "nebraska"), colour = "grey20") +
  coord_sf(default_crs = sf::st_crs(4326), ylim = c(10, 50)) +
  theme_void() +
  theme(legend.position = "none") +
  scale_fill_manual(values = c("TRUE" = "#EE2C2C", "FALSE" = "#FFF8DC")) 

# Add the magnify layer

ggpm + geom_magnify(aes(from = ID == "nebraska"),
                    to = c(-125, -98, 15, 35), 
                    shadow = TRUE, linewidth = 1, colour = "grey20",
                    shape = "outline", 
                    aspect = "fixed", 
                    expand = 0) 
...
## State vs Federal Law {background-image="images/constitution.jpg" background-opacity="0.2"}
...
tbl <- j_data |> 
  ...
  gt_plt_bar(column = first, color = "#005d84", keep_column = TRUE, width = 40, scale_type = "none", text_color = "white") 

# Reshape the j_data tibble
ex_df <- j_data |> 
  pivot_longer(cols = c(first, second), names_to = "Measure", values_to = "data") |> 
  mutate(Measure = ifelse(Measure == "first", "2017", "2022"))

# Create a summary data frame for gt_plt_bar_stack
tab_df <- ex_df |> 
  group_by(journal) |> 
  summarise(list_data = list(data))

# Create a gt table with gt_plt_bar_stack
tab_df |> 
  gt() |> 
  gt_theme_dark() |>
    gt_highlight_rows(rows = 6, fill = "gray",
    bold_target_only = TRUE,) |> 
  gt_plt_bar_stack(column = list_data, palette = c("#d00000", "#005d84"), labels = c("2017", "2022"),
)
...
## Method {background-image="images/file.png" background-opacity="0.1"}
...
## Coded Variables {background-image="images/process.png" background-opacity="0.1"}
...
-   **Connection to Law**
    ...
    -   References to statute(s)

    -   References to state or fed constitutions
...
## Primary Research Areas {.inverse}
...
bar_data <- lhb |> 
  janitor::clean_names() |> 
   mutate (gen_area = as.character(gen_area),
# Create factor decade variable 
  gen_area = case_when(
      gen_area=="1" ~ "jury decision making",
      gen_area=="2" ~ "eyewitness memory",
      ...
      gen_area=="21" ~ "mental health",
      gen_area=="24" ~ "mental health"),
    gen_area = factor(gen_area, levels=c("jury decision making", "eyewitness memory", "child witnesses", "expert witnesses", "risk assessment", "public policy", "juvenile law", "mental health"))
  ) |> 
    ## remove NA values
  filter(!is.na(gen_area)) 

bar_data
```
bar_data |> 
  mutate(gen_area = stringr::str_to_title(gen_area))  |> 
  summarize(prop = n() / nrow(bar_data))  |> 
  mutate(gen_area = forcats::fct_reorder(gen_area, -prop))  |> 
  ggplot(aes(prop, gen_area)) +
  geom_col(aes(fill = as.numeric(gen_area) == 1)) +
  theme_void() +
  scale_x_continuous(
    name = "Percentage of articles", expand = c(0, 0),
    limits = c(0, 0.42), labels = scales::label_percent()
  ) +
  scale_y_discrete(
    name = "Sample used",
  ...
  scale_fill_manual(
    values = c("#B0B0B0D0", "#ff8281"),
    guide = "none"
  ) +
  theme(
  legend.position = element_blank(),
  axis.text.y = element_text(color = "white", family = "Architects Daughter", size = 14, hjust = 1),
  plot.margin = margin(rep(15, 4)),
  plot.background = element_rect(fill = "#242423", color = "#aa2233", size = 3)
  )
...
## Research Areas Over Time :hourglass_flowing_sand: {.smaller}

::: columns
::: {.column width="50%"}
![](images/lhbold.jpg){fig-align="center" width="217"}
:::
...
---
![](images/areas-time.gif){.r-stretch}

## :hourglass_flowing_sand: Witnesses
...
linecomplete %>%
  ggplot( aes(x=year, y=count, color=gen_area)) +
    ...
    theme_ipsum() +
    theme(legend.text = element_text(size=12, family = "Architects Daughter"),
          legend.title = element_text(size=14, family = "Architects Daughter"),
...
## :hourglass_flowing_sand: Risk Assessment
...
## :hourglass_flowing_sand: Mental health
...
## :hourglass_flowing_sand: Public Policy
...
## :hourglass_flowing_sand: Juvenile Law
...
## :hourglass_flowing_sand: Jury decision-making
...
## :hourglass_flowing_sand: Eyewitness :chart_with_upwards_trend: \> Juries :chart_with_downwards_trend:
...
## :hourglass_flowing_sand: Citations Over Time

![](images/animated-line.gif){fig-align="center"}
...
## Most Cited (Top 20) Articles {.scrollable .inverse}

```{r}
#| label: Create table with highest citation counts


# Create a tibble with the data
data <- tribble(
  ~cite, ~title, ~topics, ~first, ~second,
...
tbl <- data |> 
  gt() |> 
  gt_theme_dark() |> 
  gt_merge_stack(col1 = cite, col2 = title, palette = c("white", "grey")) |> 
  gt_plt_bar(column = first, color = "#005d84", keep_column = FALSE, width = 40, scale_type = "number", text_color = "white") |> 
  gt_plt_bar(column = second, color = "#d00000", keep_column = FALSE, width = 40, scale_type = "number", text_color = "white") |> 
  cols_label(
        cite = "Author(s) (year)",
  ...
  tab_spanner(
    label = md('**Number citations**'),
    columns = 4:5
  ) 
# |> tab_header(title = add_text_img("20 Most cited papers", url = "https://upload.wikimedia.org/wikipedia/en/a/a1/Law_and_Human_Behaviour_journal_cover.jpg", left = TRUE))
...
sample_bar |>
  ...
  geom_text(
    ...
    color = "white", size = 5, family = "Architects Daughter", fontface = "bold"
  ) +
  scale_x_continuous(expand = c(.01, .01)) +
  scale_fill_identity(guide = "none") +
  theme_void() +
  theme(
    axis.text.y = element_text(color = "white", size = 14, hjust = 1, family = "Architects Daughter"),
    plot.margin = margin(rep(15, 4)),
    panel.grid = element_blank(),
    plot.background = element_rect(fill = "#242423",
                                color = "#aa2233",
                                size = 3),
...
cbPalette <- c("#c1121f", "#fdf0d5", "#999999", "#001d3d", "#003566", "#0072B2", "#ffc300", "#ffd60a","#780000")

# Convert the "names" variable to a factor and order it based on the "n" variable using fct_reorder
method$names <- fct_reorder(method$names, method$n)
...
method_bar |>
  ...
  theme_void() +
     theme(panel.background = element_blank(),
        plot.background = element_rect(fill="#242423", color="#aa2233", size = 3),
        plot.margin = margin(rep(15, 4)),
        text = element_text(family = "Architects Daughter", color="white"),
        panel.grid = element_blank(),
...
law_bar |>
  ...
  geom_text(
    aes(label = perc, hjust = place), 
    color = "white", size = 7, family = "Architects Daughter", fontface = "bold"
  ) +
  scale_x_continuous(expand = c(.01, .01)) +
  scale_fill_identity(guide = "none") +
  theme_void() +
  theme(
    axis.text.y = element_text(color = "white", size = 14, family = "Architects Daughter", hjust = 1),
    plot.margin = margin(rep(15, 4)),
    plot.background = element_rect(fill = "#242423",
                                color = "#aa2233",
                                size = 3),
...
## Reference to Legal Doctrine {.inverse}
...
law1 <- none_bar |>
  ...
  geom_text(
    aes(label = perc), 
    hjust = 1, nudge_x = -.5,
    size = 3.5, family = "Architects Daughter", fontface = "bold"
  ) +
  scale_x_continuous(expand = c(.01, .01)) +
  scale_fill_identity(guide = "none") +
  theme_void() +
  theme(
    axis.text.y = element_text(color = "white", size = 12, family = "Architects Daughter", hjust = 1),
    plot.margin = margin(rep(15, 4)),
    plot.background = element_rect(fill = "#242423",
                                color = "#aa2233",
                                size = 3),
...
## 78% applied psychological theory {.inverse}
...
crt <- waffle_icon(
  values = c(rep("Yes", 78), rep("No", 22)), 
  colors = list(
    Yes = "#005d84",
    No = "#F45B69"
  ),
  icons = list(
    Yes = ph("thumbs-up", weight = "bold"),
    No = ph("x", weight = "bold")
  ),
  ncol = 12,
  nrow = 8,
...
## Application of Psychological Theory {.inverse}
...
# Questions? {background-image="images/uslawpsych.png" background-opacity="0.1"}
